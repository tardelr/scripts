...
ASSISTANT_REFERENCE_DICT = {
    'Mavi - Mosaic': 'assistant'  # Using regular dash to match normalized version
    }
...
def parse_to_context(messages: list):
    ...
    for message in messages:
        # Normalize sender string for dictionary lookup
        sender = message["sender"]

        context.append({
            "role": ASSISTANT_REFERENCE_DICT.get(sender, "user"),
...
def write_context_to_csv(rows, output_path):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        ...
        print(f"Wrote {len(rows)} context messages to {output_path}")
...
def main():
    ...
    for file in files:
        ...
        base_name = file.stem + "_parsed.csv"
        
        # Paths for each type of output
        parsed_output_path = parsed_dir / base_name
        context_output_path = context_dir / f"context_{base_name}"

        raw = file.read_text(encoding="utf-8-sig")
        rows = parse_chat(raw)
        context = parse_to_context(rows)

        write_messages_to_csv(rows, parsed_output_path)
        write_context_to_csv(context, context_output_path)
